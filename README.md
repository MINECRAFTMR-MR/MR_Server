...
##❗️注意事项
- 1. 请注意任何形式进行倒卖或者售卖的形式，本端永远免费，如果你要出售，请标明免费与标明作者。
- 2. 因用户操作不当导致的文件或数据损坏，丢失我们将不会承担任何责任。
- 3. 允许使用端内内容为自己服务端进行修改，但不得将修改后的服务端内容或提取服务端配置内容进行售卖。
- 4. 服务器用的是26.1.2，需要安装jdk25。
- 5. 只需要安装重要插件，列表里的东西就可以，
安装其他的容易崩溃。
- 6. 如果你是以付费的形式买到本端，请立即要求退款。
